...
**Quick Start** asks two questions — book size (6×9, 5.5×8.5, 5×8) and look (Classic, Modern,
Minimal) — and builds the design itself. The nine designs are generated by the app as real `.docx`
files: no Amazon template is redistributed, and no placeholder prose ships inside one. Each carries
proper styles, mirrored margins, a binding gutter and a page-number footer.
...
Optional details — title, subtitle, author, copyright year, publisher, ISBN, dedication,
acknowledgments, an About the Author note, a bibliography — are used to build those pages in the
chosen design. A section you leave blank is skipped rather than producing a headed empty page, and
